h2. Jiraclient User Manual
==<hr />==

p. A python command line client for Atlassian's Jira issue tracker.

h3. Templates

Create a set of issues with a YAML formatted template file like this:

<pre class="terminal">jiraclient --template project.yaml</pre>
...
stories:
  - summary: s1 summary
    description: story s1 has is a very long description for a jira story that will keep on going and going and going
    timetracking: 1h
    subtasks:
...
p. Usage: jiraclient [options]

p. Sample Usage:

* Standard issue creation in project named INFOSYS:
<p class="terminal">jiraclient -u 'username' -p 'jirapassword' -A 'auser' -P INFOSYS -T task -S 'Do some task'</p>

* Get numerical Version IDs for Project named INFOSYS:
<p class="terminal">jiraclient -u 'username' -p 'jirapassword' -a getVersions INFOSYS</p>

* Get numerical Component IDs for Project named INFOSYS:
<p class="terminal">jiraclient -u 'username' -p 'jirapassword' -a getComponents INFOSYS</p>

* Create an issue with a specified Component and Fix Version and assign it to myself:
<p class="terminal">jiraclient -u 'username' -p 'jirapassword' -A 'username' -P INFOSYS -Q major -F 10000  -C 10003 -T epic -S 'Investigate Platform IFS'</p>
